...
import { InspectorPanel, LanguageToggle, useLocale } from '@datapass/ui';
...
export default function App() {
  ...
  return <div className="architecture-app"><a className="architecture-skip" href="#main">Skip to content</a><header className="architecture-top"><strong>Architecture Atlas</strong><span>Stage first. Provider second.</span><LanguageToggle /></header><main id="main" className="architecture-main">
    ...
    <section className="architecture-source"><h2>Sources & freshness</h2><p>Sixteen workload/provider variants adapted from an author-provided architecture reference. Public attribution: {architectureSource.id}. Full private-source provenance is retained in the audit reports, not this application.</p><p>Pinned source snapshot: {architectureSource.verifiedAt}. Provider terminology is source-reported, not live-verified. Confirm current service availability before deploying. All diagrams are local explanatory models; no backend, Spark execution or SQL parser.</p></section>
  </main></div>;
}
