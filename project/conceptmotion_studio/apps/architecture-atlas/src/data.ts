...
export function architectureDiagram(workload: Workload, provider: Provider, layout: 'layered' | 'radial'): DiagramSpec & { frames: readonly { id: string; activeNodeIds: readonly string[]; activeEdgeIds: readonly string[] }[] } {
  ...
  return { kind: 'diagram', version: '3', id: `architecture-${workload}-${provider}-${layout}`, title: workloadNames[workload], description: 'Source → Move → Store → Process → Model → Serve', layout: { provider: layout, hubId: 'process' }, nodes: stages.map((stage, i) => ({ id: stage, label: labels[i], kind: stage, preferredRank: i < 6 ? i : i === 6 ? 2 : 3, iconId: stage === 'store' ? 'data.table' : stage === 'source' ? 'data.source' : 'generic.node', metadata: { stage, intent: stageIntent[stage], sourceId: architectureSource.id } })), edges: [...edges, { id: 'operate-process', from: { nodeId: 'operate' }, to: { nodeId: 'process' }, flowKind: 'control' }, { id: 'govern-store', from: { nodeId: 'govern' }, to: { nodeId: 'store' }, flowKind: 'lineage' }], frames: stages.map((stage, i) => ({ id: stage, activeNodeIds: [stage], activeEdgeIds: i > 0 && i < 6 ? [`${stages[i - 1]}-${stage}`] : stage === 'operate' ? ['operate-process'] : stage === 'govern' ? ['govern-store'] : [] })) };
}
