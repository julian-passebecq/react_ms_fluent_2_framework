...
export default function CatalogPage({ progress, onSelect }: { progress: ProgressStateV2; onSelect: (id: string) => void }) {
  ...
  const facet = (key: string, label: string, options: { value: string; label: string }[]) => <Field label={label}><Select aria-label={label} value={state.filters[key]?.[0] ?? ''} onChange={event => setState(current => setCatalogFacetValues(current, key, event.target.value ? [event.target.value] : []))}><option value="">All</option>{options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</Select></Field>;
