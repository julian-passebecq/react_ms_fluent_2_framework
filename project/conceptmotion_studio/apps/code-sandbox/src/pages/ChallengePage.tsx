...
import { figureForPracticeId, visualById } from '../../../../content/visuals';
...
export default function ChallengePage({ id, workspace, onBack }: { id: string; workspace: ReturnType<typeof usePracticeWorkspace>; onBack: () => void }) {
  ...
  return <><div className="sandbox-actions"><Button onClick={onBack}>Back to practice</Button><Button disabled={index === 0} onClick={() => navigate(-1)}>Previous item</Button><Button disabled={index === practiceItems.length - 1} onClick={() => navigate(1)}>Next item</Button></div><ChallengeWorkbench key={id} challenge={item} figure={figure} figureCaptions={visual?.captions} locale={locale} reducedMotion={reducedMotion} progress={workspace.state.progress} onProgressChange={workspace.updateProgress} notes={workspace.state.notes[id]} onNotesChange={notes => workspace.setNote(id, notes)} /></>;
}
