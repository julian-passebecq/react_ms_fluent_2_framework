...
export function App() {
  ...
  const sideNav = (
    <SideNav
      title={locale === 'no' ? 'Læringsstier' : 'Learning paths'}
  ...
  return (
    <AppShell
      topBar={(
        <TopBar
          brand="Dubreu Formation"
          subtitle={title}
          localeControl={<LanguageToggle />}
          actions={<span className="formation-local-label">LOCAL PROGRESS · NO EXECUTION</span>}
          actionsLabel={locale === 'no' ? 'Applikasjonshandlinger' : 'Application actions'}
        />
      )}
      sideNav={sideNav}
      lang={locale === 'no' ? 'nb' : 'en'}
      mainLabel="Dubreu Formation learning workspace"
