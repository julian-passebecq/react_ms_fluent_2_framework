...
Foundation V2 composes reusable content, import, figure, learning, progress, catalog, and app-generation contracts on top of the preserved Foundation v1.1 semantic/rendering packages. The Studio and Dubreu Formation are consumers of those contracts; neither application is a library boundary.
...
@datapass/notebook-import ---> @datapass/content <--- @datapass/scaffold
                                      ^
                                      |
apps/dubreu-formation ---> @datapass/learning ---> @datapass/code ---> Monaco
              |                   |  |  |
              |                   |  |  `----> @datapass/progress
              |                   |  `-------> @datapass/figure
              |                   `----------> @datapass/content
...
The repository workflow at `.github/workflows/ci.yml` installs a frozen lockfile and runs TypeScript, unit coverage, boundary checks, scaffold smoke, Studio/bundle, Dubreu, legacy, Storybook, and desktop/phone browser gates. A workflow file is not evidence of a hosted green run; exact local and hosted results belong in the V2 test report.
...
## Intentionally outside Foundation V2
