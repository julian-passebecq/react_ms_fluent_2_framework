...
Independent integration review found and fixed two new edge cases: unchecked non-string Figure status must not be rendered as a React child, and opt-in sizing must not bypass existing malformed-scene fallback. Visual inspection additionally corrected split-pane text shrinkage, oversized layered canvases, radial node scale and an over-tall Interview prelude. Initial hosted V4 CI found two semantic titles overflowing with Linux fallback fonts; the follow-up uses width-budget wrapping with an explicit locally reproduced wider-font regression, preserving the same font size and containment thresholds. These corrections and exact follow-up verification are recorded in the test report.
